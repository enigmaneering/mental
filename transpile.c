...
mental_language mental_detect_language(const char* source, size_t source_len) {
    ...
    /* GLSL detection — check early if #version is present, since it's
     * unambiguous and prevents false positives from identifiers that
     * contain HLSL/WGSL keywords (e.g. "type_RWStructuredBuffer_float"
     * generated by spirv-cross from DXC's SPIR-V). */
    ...
    /* GLSL detection (remaining patterns — #version already checked above) */
    if (contains(source, source_len, "layout(") ||
...
mental_language mental_api_to_language(mental_api_type api) {
    switch (api) {
        ...
        case MENTAL_API_OPENCL: return MENTAL_LANG_GLSL; /* Backend handles GLSL → OpenCL C transpilation */
        case MENTAL_API_OPENGL: return MENTAL_LANG_GLSL; /* OpenGL 4.3+ uses GLSL compute */
        case MENTAL_API_POCL:   return MENTAL_LANG_GLSL; /* Backend handles GLSL → OpenCL C transpilation */
        case MENTAL_API_WEBGPU: return MENTAL_LANG_WGSL;  /* WebGPU uses WGSL natively */
