...
unsigned char* mental_hlsl_to_spirv_direct(const char* source, size_t source_len,
    ...
    *out_len = 0;
    ...
    /* DXC's library initialization runs via __attribute__((constructor)) on
     * DllMain() in DXCompiler.cpp. Emscripten supports constructors from
     * static libraries, so the thread allocator should be initialized before
     * we reach here. If it isn't, DxcCreateInstance will fail with an HRESULT
     * and we report that gracefully below. */
